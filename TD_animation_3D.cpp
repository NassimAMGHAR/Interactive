...
void DirectKinematics(double deltaT)
{
	...
	if (selected != -1) {                     
		if (keyUp)	     Skelet[selected].SetGamma( Skelet[selected].GetGamma() + rotationSpeed * deltaT);
		if (keyDown)	 Skelet[selected].SetGamma( Skelet[selected].GetGamma() - rotationSpeed * deltaT);
		if (keyLeft)	 Skelet[selected].SetBeta ( Skelet[selected].GetBeta()  + rotationSpeed * deltaT);
		if (keyRight)	 Skelet[selected].SetBeta ( Skelet[selected].GetBeta()  - rotationSpeed * deltaT);
		if (keyPgUp)	 Skelet[selected].SetAlpha( Skelet[selected].GetAlpha() + rotationSpeed * deltaT);
		if (keyPgDown)	 Skelet[selected].SetAlpha( Skelet[selected].GetAlpha() - rotationSpeed * deltaT);	}
